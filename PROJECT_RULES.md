...
**Macro regime diagnostics** (`stress_report.json.macro_regime_diagnostics`) are diagnostic-only and non-binding. Method version `macro_two_axis_v1` is a **monthly** two-axis macro classifier (see `docs/docs/stress_testing_spec.md` §8.8.2) producing a `growth_score`, an `inflation_score`, and one of five labels (`goldilocks`, `reflation`, `stagflation`, `recession_disinflation`, `neutral_transition`) with a 1-month publication lag. Indicators (PAYEMS, UNRATE, real PCE/DPI, HY OAS, NFCI, core/headline CPI, core PCE, AHE, ECI, breakevens, plus optional ISM and GDPNow as the sole nowcast indicator after NY Fed Nowcast was retired on 2026-05-07) flow through a layered source resolver (FRED → Yahoo → official CSV → official API → keyed API → manual CSV) so missing sources degrade `coverage_tier` and `confidence_level` without crashing the run. Per-regime monthly OLS regression, factor covariance, factor risk and RC are gated by n_obs (`<12` `insufficient_data`, `12–23` `low_confidence` with linear shrinkage to `base_10y`, `24–59` `usable`, `60+` `reliable`). The block must not replace optimizer inputs, mandate gates, stress pass/fail logic, weight release, or raw 5Y/10Y beta outputs.
