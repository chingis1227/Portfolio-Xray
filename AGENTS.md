...
Scenario-Based Robust Optimization v1 (requires **`scenario_library_normalized.json`** from the report; does not run mandate gates or overwrite **`portfolio_weights.yml`**):
```bash
...
Run **Risk Budgeting** baselines (benchmark-only; same eligible universe and Ledoit–Wolf monthly **Σ** + PSD repair as Risk Parity; `risk_budgeting` in `config.yml`):

- **By taxonomy bucket (SLSQP):** matches aggregated percentage contributions to variance to preset or manual class targets (`run_risk_budget_by_asset_class.py` → `risk budget by asset-class portfolio/`).
- **By asset (Spinu CCD, SLSQP fallback):** requires `risk_budgeting.asset_targets` on every eligible ticker, sums to 1 (`run_risk_budget_by_asset.py` → `risk budget by asset portfolio/`).
...
**Technical.** **SLSQP** on ``min λ·w′Σw − μ′w`` with ``sum(w)=1``. Baseline CLIs resolve λ from ``analysis_robust_mv_lambda_calibration/selected_lambda.txt`` after ``run_robust_mv_lambda_calibration.py``, or from ``--robust-mv-lambda``; YAML does **not** supply λ (omit ``robust_mv_lambda`` or leave unset). Tests may still ``replace(cfg, robust_mv_lambda=…)``. **No** ProLiquidity / mandate overlays on the baseline scripts themselves.
...
Calibrate **`robust_mv_lambda`** against IPS targets from `config.yml` using **`run_robust_mv_lambda_calibration.py`** (YAML ``robust_mv_lambda`` is **not** read by this tool). Primary grid default ``0.1…1.0``; if no mandate-eligible λ, automatically evaluates extended ``1.5…10``. Summary JSON includes ``primary_grid_tested``, ``extended_grid_tested``, ``feasible_found_in_primary_grid``, ``feasible_found_in_extended_grid``, ``selected_lambda``, ``selected_lambda_source`` (``primary_grid`` | ``extended_grid`` | ``least_bad``), ``failed_constraints_by_grid``, and ``extended_search_explanation`` when the extended phase ran or least-bad fallback applies. Writes ``analysis_robust_mv_lambda_calibration/robust_mv_lambda_calibration.csv``, ``robust_mv_lambda_calibration_summary.json``, ``selected_lambda.txt``, ``selected_weights.json``, and a full ``selected_portfolio/`` report when a build exists at the chosen λ. When none qualifies, the summary JSON adds **`no_feasible_lambda_diagnostic`** (tested λ range, fallback λ / build status, breached limits at that candidate, generic causes, suggested actions) and **`selection_note`** carries the narrative text; logs emit a warning. Does **not** change the policy optimizer or internal stress gate code paths.
...
- `config/historical_stress_proxy_map.yml` - default ticker / asset-class proxy map and coverage thresholds for historical stress fallback tiers (`src/historical_stress_fallback.py`).
...
- `src/portfolio_variants.py`, `src/risk_budgeting.py`, `src/risk_budgeting_presets.py`, `src/risk_parity_spinu.py`, `src/hrp_weights.py` - Equal-Weight / Risk-Parity / HRP / Risk Budgeting / Minimum-Variance baseline builders (`run_equal_weight.py`, `run_risk_parity.py`, `run_risk_budget_by_asset_class.py`, `run_risk_budget_by_asset.py`, `run_hierarchical_risk_parity.py`, `run_minimum_variance.py`); RP weights via Spinu CCD on Ledoit-Wolf covariance with SLSQP emergency fallback; **risk budgeting by class** minimizes squared error of **aggregated** percentage variance contributions vs taxonomy buckets (SLSQP; merged `etf_universe` + `stock_universe`); **risk budgeting by asset** uses Spinu CCD with unequal budgets (`asset_targets`) plus SLSQP fallback; **canonical HRP** via correlation-distance clustering and recursive bisection (no bounds, no projection); MV minimizes `0.5 w'Σw` with SLSQP + `Σw` gradient on PSD-repaired **Σ**, same box bounds as the policy optimizer when `young_etf_optimization_policy` / `covariance_shrinkage` align with `run_optimization.py`. Maximum Diversification baselines: **constrained** (`run_maximum_diversification.py`) maximizes `(σ'w)/√(w'Σw)` on the same monthly **Σ** path and **`_build_bounds`** box via SLSQP; **unconstrained long-only** (`run_maximum_diversification_unconstrained.py`, `maximum_diversification_unconstrained`) maximizes the same ratio with only `sum(w)=1` and per-asset `[0,1]` bounds (no policy min/max, feasibility caps, or Young caps as optimizer constraints). **Minimum CVaR** (`run_minimum_cvar_uncapped.py`, `run_minimum_cvar_constrained.py`): Rockafellar–Uryasev LP via `scipy.optimize.linprog` (HiGHS) on monthly scenarios — uncapped uses `w_i∈[0,1]` without project caps; constrained uses **`_build_bounds`**. Config **`minimum_cvar_confidence_level`** (default `0.95`). **Robust Mean–Variance** (`run_robust_mean_variance_uncapped.py`, `run_robust_mean_variance_constrained.py`, `run_robust_mv_lambda_calibration.py`, `src/robust_mv.py`, `src/robust_mv_calibration.py`): stabilized-input mean–variance benchmark (James–Stein μ; sklearn Ledoit–Wolf or OAS Σ; λ on monthly variance); Young dual affects **bounds only** on the constrained path; uncapped uses `[0,1]` only; constrained uses **`_build_bounds`**. IPS-aligned λ from **`run_robust_mv_lambda_calibration.py`** (YAML **`robust_mv_lambda`** is **not** used by calibration or baseline CLIs); baseline runners read **`analysis_robust_mv_lambda_calibration/selected_lambda.txt`** or **`--robust-mv-lambda`**; programmatic **`replace(cfg, robust_mv_lambda=…)`** for tests. Metadata **`robust_mv_variant_role`** / **`robust_mv_variant_summary`**. Config **`robust_mv_covariance_method`** (`ledoit_wolf` | `oas`), **`robust_mv_mu_shrinkage_method`** (`james_stein` only). **Not** the policy optimizer replacement—see AGENTS / SPEC variant interpretation.
...
- `src/scenario_library.py` — **Scenario Library v1** (`scenario_library_v1`): input-standardization only; after `build_stress_scenario_analytics`, `run_report.py` / `run_portfolio_report_for_weights` builds normalized scenario objects from base + full `regime_factor_analytics` payload + stress layers, writes `scenario_library.json` to `output_dir_final` and `scenario_library_summary.csv`, `scenario_library_missing_inputs.csv`, `scenario_library_warnings.csv` under `results_csv/`; exposes `stress_report.scenario_library_meta` (no optimizer/mandate/stress-gate changes).
- `src/scenario_library_normalized.py` — **Scenario Library Normalized View v1** (`scenario_library_normalized_v1`): derived optimization-input view on top of Scenario Library v1; upstream analytics unchanged. Monthly base slice with **`expected_return_method`** (`historical_monthly_mean` or `optimizer_mu_precomputed`), **`pipeline_returns_frequency_note`** vs **`optimization_frequency`**, normalized readiness classification (synthetics/historical/base/macro rules), optional **`enable_historical_stress_fallback`** path wiring per-asset episode returns + **`historical_stress_metadata`**, **`readiness_roles`** in `stress_report.scenario_library_normalized_meta`; artifacts as listed in SPEC.
- `src/historical_stress_fallback.py` — per-asset historical episode return waterfall (direct → ticker proxy → asset-class proxy → factor replay) for normalized-library historical rows; does not overwrite direct ETF history. Tier 4 for episodes ending before **2007-01-01** uses a long weekly factor panel from `run_report.py` (`build_factor_matrix` from 1990 with `require_complete_rows=False`) passed via `factor_returns_weekly_episode_loose` into `build_scenario_library_normalized`; later episodes use the strict **2007+** matrix (same as `run_stress` / scenario library).
- `src/robust_scenario_optimization.py` — **`robust_scenario_optimization_v1`**: builds scenario coefficient matrix from normalized JSON + **`stress_report.json`** **`asset_factor_betas`** (weekly 5Y per-ticker map; explicit portfolio-beta replication fallback if absent), **lower-half-mean** / maximin / hybrid objectives, **SLSQP** multi-start, CSV/JSON exporters (`beta_load_warnings` in summary when fallbacks fire).
- `run_robust_scenario_optimization.py` — CLI for robust scenario optimization; optional YAML **`robust_scenario_optimization`** block in `config.yml` (`normalized_json_path`, `output_dir`, `lambdas`).
