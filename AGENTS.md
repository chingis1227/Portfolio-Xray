...
Run **Robust Mean–Variance** baselines (James–Stein shrunk μ; Ledoit–Wolf or OAS shrunk monthly Σ; **SLSQP** on `min λ·w′Σw − μ′w` with `sum(w)=1`; config `robust_mv_lambda` default `0` = no variance penalty, maximize shrunk μ subject to bounds only; **no** ProLiquidity / mandate overlays):
