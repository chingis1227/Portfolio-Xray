...
| Frontend IA/copy regression | Primary UI copy, route chrome, badge discipline, defensive-copy limits, or screen information architecture changed | `cd frontend && npm.cmd run test:copy`; pass requires product guardrails to stay out of primary surfaces as repeated defensive copy |
...
After changing the live Run Diagnostics macro/PCA boundary, run focused report/context tests and
inspect the generated warm `analysis_subject/stress_report.json`. It must contain normal
stress/factor/scenario evidence but no `macro_regime_diagnostics`, `regime_factor_analytics`,
`regime_portfolio_metrics`, `portfolio_pca`, `pc1_explained_variance_ratio`, `residual_pca`, or
`pca_pc1_*` evidence. Do not assert that the Stress Lab scenario id `inflation_stagflation` is
absent; it is a stress scenario, not the removed macro regime classifier.
