...
def test_problem_classification_v3_contract_accepts_facade_output() -> None:
    ...
    assert doc["schema_version"] == PROBLEM_CLASSIFICATION_V3_VERSION
...
def test_problem_classification_v3_rejects_v1_schema() -> None:
    violations = problem_classification_v3_product_contract_violations(
...
def test_block_4_v3_handoff_links_launchpad_to_problem_classification(tmp_path: Path) -> None:
...
def test_live_e2e_diagnosis_only_validates_block_4_v3_contract(tmp_path: Path) -> None:
    ...
    assert result.evidence.get("block_4_schema_version") == PROBLEM_CLASSIFICATION_V3_VERSION
...
def test_live_e2e_fails_block_4_when_launchpad_v2_contract_broken(tmp_path: Path) -> None:
    ...
    assert any("candidate_launchpad_v3" in err for err in result.errors)
