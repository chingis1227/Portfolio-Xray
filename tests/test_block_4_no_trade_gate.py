"""Tests for Block 4 v3 no-trade / monitor gate (Session 09)."""
...
from src.block_4.launchpad_cards import build_candidate_launchpad_v3_document
...
def test_no_trade_view_passes_problem_classification_contract_stub() -> None:
    ...
    doc = result.problem_classification
    violations = problem_classification_v3_product_contract_violations(doc)
    assert not violations
    assert doc["summary"]["no_trade_outcome"] == result.gate.outcome
...
def test_no_trade_gate_ruleset_version_constant() -> None:
    assert NO_TRADE_GATE_RULESET_VERSION.startswith("block_4_v3_")
