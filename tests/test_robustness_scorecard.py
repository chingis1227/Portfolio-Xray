from __future__ import annotations

import json
from pathlib import Path

from src.robustness_scorecard import (
    SCHEMA_VERSION,
    build_robustness_scorecard,
    write_robustness_scorecard_outputs,
)
from src.config_schema import validate_config


def _comparison_three_candidates() -> dict:
    base_metrics = {
        "cagr": 0.08,
        "vol_annual": 0.12,
        "max_drawdown": -0.2,
        "sharpe": 0.5,
        "sortino": 0.6,
    }
    return {
        "schema_version": "candidate_comparison_v1",
        "primary_window": "10y",
        "candidates": [
            {
                "candidate_id": "alpha",
                "display_name": "Alpha",
                "status": "available",
                "metrics": {"10y": {**base_metrics, "cagr": 0.10, "sharpe": 0.7}},
                "stress": {
                    "overall": "DIAG_PASS",
                    "scenarios": [
                        {"scenario_id": "equity_shock", "portfolio_pnl_pct": -0.04, "pass": True},
                        {"scenario_id": "credit_shock", "portfolio_pnl_pct": -0.02, "pass": True},
                    ],
                },
                "drawdown": {"max_drawdown": -0.15, "recovered": True, "time_to_recovery_months": 6},
                "diversification": {
                    "top1_rc_asset": "VOO",
                    "top1_rc_pct": 0.25,
                    "top3_rc_assets": ["VOO", "BND", "GLD"],
                    "top3_rc_sum_pct": 0.55,
                    "source_window": "10y",
                },
                "mandate": {"portfolio_valid": True, "client_fit": True},
                "factor_regime": {
                    "factor_regression_10y": {
                        "adj_r_squared": 0.8,
                        "betas": {"eq": 0.5, "credit": 0.1},
                    }
                },
                "warnings": [],
            },
            {
                "candidate_id": "bravo",
                "display_name": "Bravo",
                "status": "available",
                "metrics": {"10y": {**base_metrics, "cagr": 0.06, "sharpe": 0.4}},
                "stress": {
                    "overall": "DIAG_ATTENTION",
                    "scenarios": [
                        {"scenario_id": "equity_shock", "portfolio_pnl_pct": -0.12, "pass": False},
                    ],
                },
                "drawdown": {"max_drawdown": -0.28, "recovered": False},
                "diversification": {
                    "top1_rc_asset": "QQQ",
                    "top1_rc_pct": 0.40,
                    "top3_rc_assets": ["QQQ", "VOO", "BND"],
                    "top3_rc_sum_pct": 0.80,
                    "source_window": "10y",
                },
                "mandate": {"portfolio_valid": True, "client_fit": True},
                "factor_regime": {},
                "warnings": [],
            },
            {
                "candidate_id": "charlie",
                "display_name": "Charlie",
                "status": "available",
                "metrics": {"10y": {**base_metrics, "cagr": 0.09, "sharpe": 0.55}},
                "stress": {"overall": "DIAG_PASS_WITH_WARNING", "scenarios": []},
                "drawdown": {"max_drawdown": -0.18},
                "diversification": {
                    "top1_rc_asset": "BND",
                    "top1_rc_pct": 0.30,
                    "top3_rc_sum_pct": 0.65,
                    "top3_rc_assets": ["BND", "VOO", "GLD"],
                    "source_window": "10y",
                },
                "mandate": {"portfolio_valid": False, "client_fit": False},
                "factor_regime": {},
                "warnings": [],
            },
        ],
    }


def test_schema_required_keys() -> None:
    doc = build_robustness_scorecard(_comparison_three_candidates())
    assert doc["schema_version"] == SCHEMA_VERSION
    assert doc["diagnostic_only"] is True
    for key in (
        "generated_at",
        "weights_profile",
        "weights",
        "primary_window",
        "input_artifact",
        "candidates",
        "comparison_summary",
        "warnings",
    ):
        assert key in doc


def test_relative_ranking_three_candidates() -> None:
    doc = build_robustness_scorecard(_comparison_three_candidates())
    by_id = {c["candidate_id"]: c for c in doc["candidates"]}
    assert by_id["alpha"]["total_score"] > by_id["bravo"]["total_score"]
    assert by_id["alpha"]["robustness_rank"] == 1
    assert doc["comparison_summary"]["highest_robustness_candidate_id"] == "alpha"


def test_tie_breaking_alphabetical() -> None:
    comp = _comparison_three_candidates()
    for c in comp["candidates"]:
        if c["candidate_id"] in ("alpha", "bravo"):
            c["metrics"]["10y"] = comp["candidates"][0]["metrics"]["10y"]
            c["stress"] = comp["candidates"][0]["stress"]
            c["diversification"] = comp["candidates"][0]["diversification"]
            c["drawdown"] = comp["candidates"][0]["drawdown"]
            c["mandate"] = comp["candidates"][0]["mandate"]
    doc = build_robustness_scorecard(comp)
    ranked = doc["comparison_summary"]["ranking_table"]
    tied = [r for r in ranked if r["candidate_id"] in ("alpha", "bravo")]
    if len(tied) == 2 and tied[0]["total_score"] == tied[1]["total_score"]:
        assert tied[0]["candidate_id"] == "alpha"


def test_unavailable_not_scored() -> None:
    comp = _comparison_three_candidates()
    comp["candidates"].append(
        {
            "candidate_id": "missing",
            "display_name": "Missing",
            "status": "unavailable",
            "unavailable_reason": "missing_artifact_folder",
        }
    )
    doc = build_robustness_scorecard(comp)
    row = next(c for c in doc["candidates"] if c["candidate_id"] == "missing")
    assert row["score_status"] == "not_scored"
    assert row["total_score"] is None


def test_mandate_invalid_caps_component() -> None:
    doc = build_robustness_scorecard(_comparison_three_candidates())
    charlie = next(c for c in doc["candidates"] if c["candidate_id"] == "charlie")
    mandate = charlie["components"]["mandate_fit"]
    assert mandate["score"] <= 25
    assert "mandate_portfolio_invalid" in charlie["warnings"]


def test_missing_diversification_reweights(tmp_path: Path) -> None:
    comp = _comparison_three_candidates()
    comp["candidates"][1]["diversification"] = {}
    doc = build_robustness_scorecard(comp)
    bravo = next(c for c in doc["candidates"] if c["candidate_id"] == "bravo")
    assert bravo["components"]["diversification_rc"]["status"] == "not_computed"
    assert bravo["total_score"] is not None
    assert "diversification_inputs_missing" in doc["warnings"]


def test_single_candidate_warning() -> None:
    comp = _comparison_three_candidates()
    comp["candidates"] = [comp["candidates"][0]]
    doc = build_robustness_scorecard(comp)
    assert "single_candidate_comparison" in doc["warnings"]
    assert doc["candidates"][0]["total_score"] is not None


def test_write_outputs(tmp_path: Path) -> None:
    cfg = validate_config(
        {
            "investor_currency": "USD",
            "output_dir_final": "Main portfolio",
            "tickers": ["VOO"],
        }
    )
    out = tmp_path / "Main portfolio"
    out.mkdir()
    comparison = _comparison_three_candidates()
    with open(out / "candidate_comparison.json", "w", encoding="utf-8") as f:
        json.dump(comparison, f)

    paths = write_robustness_scorecard_outputs(cfg, project_root=tmp_path, comparison=comparison)
    assert paths["robustness_scorecard_json"].is_file()
    assert paths["robustness_scorecard_txt"].is_file()
    with open(paths["robustness_scorecard_json"], encoding="utf-8") as f:
        loaded = json.load(f)
    assert loaded["schema_version"] == SCHEMA_VERSION
