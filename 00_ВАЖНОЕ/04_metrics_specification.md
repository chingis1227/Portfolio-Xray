...
**Requirement:** In code, pass **ddof=1** explicitly wherever std/var/cov are used for these metrics and for Σ in RC_vol. For **Beta_base** and **Beta_local** always use **var(..., ddof=1)** and **cov(..., ddof=1)**; do not rely on library defaults.
...
- **Σ_window** = covariance matrix of **monthly simple returns** over the window (inner-joined, **ddof=1**).
