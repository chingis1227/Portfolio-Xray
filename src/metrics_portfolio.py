...
def portfolio_metrics_one_window(
    ...
    """Compute portfolio metrics for one window. Same definitions as asset metrics. mar=None => MAR = rf_monthly."""
    from src.windows import slice_window

    r_slice = slice_window(portfolio_returns, analysis_end, window_months)
    ...
    treynor_val = treynor(r_slice, rf_slice, bench_slice, beta_val, ddof=DDOF) if not bench_slice.empty else np.nan
    
    return {
        "window_months": window_months,
        "cagr": cagr_from_equity(r_slice, window_months),
        "vol_annual": vol_annual(r_slice, ddof=DDOF),
        "sharpe": sharpe(r_slice, rf_slice, ddof=DDOF),
        "sortino": sortino(r_slice, rf_slice, mar=mar, ddof=DDOF),
