...
"""
Build Minimum CVaR (uncapped) baseline: Rockafellar–Uryasev LP on monthly simple returns,
w_i in [0,1], sum w = 1, no project min/max or Young caps in the optimization.

Outputs under ``minimum cvar uncapped portfolio/``.
"""
...
def main() -> None:
    ...
    result = build_minimum_cvar_uncapped(
        cfg,
        monthly_returns,
        analysis_end_str,
        primary_window,
    ...
    out_dir = Path(__file__).resolve().parent / "minimum cvar uncapped portfolio"
    ...
    try:
        ...
        logger.warning("Could not compute RC_vol for Minimum CVaR uncapped baseline: %s", e)
        rc_series = None

    export_baseline_weights_txt(
        result.weights, rc_series=rc_series, label=BASELINE_MCVAR_UNCAPPED_LABEL, output_dir=out_dir
    )

    if result.status not in ("OK", "APPROXIMATE"):
        summary = {
            "portfolio_type": BASELINE_MCVAR_UNCAPPED_LABEL,
        ...
        with open(out_dir / "summary.txt", "w", encoding="utf-8") as f:
            f.write(f"{BASELINE_MCVAR_UNCAPPED_LABEL} — infeasible or failed baseline\n")
            f.write(f"Status: {result.status}\n")
            if summary.get("reason"):
                f.write(f"Reason: {summary['reason']}\n")
        print("Minimum CVaR (uncapped) baseline failed or infeasible; summary written.")
        return
    ...
    with open(out_dir / "summary.txt", "w", encoding="utf-8") as f:
        f.write(f"{BASELINE_MCVAR_UNCAPPED_LABEL}\n")
        f.write("=" * 50 + "\n\n")
        if pm_summary:
            f.write(
        ...
        md = meta_export or {}
        f.write(
            f"\nOptimizer: {md.get('optimizer_name', 'minimum_cvar_uncapped')} "
    ...
    print(f"Minimum CVaR (uncapped) baseline report written to {out_dir}")
