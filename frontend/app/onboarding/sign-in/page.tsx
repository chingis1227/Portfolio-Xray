    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-pmri-bg px-5 py-10 text-pmri-text">
      ...
      <section className="relative z-10 grid w-full max-w-5xl items-center gap-10 lg:grid-cols-[minmax(0,1fr)_420px]">
        <div>
          ...
          <p className="pmri-label mt-8 text-pmri-blueSoft">Secure workspace</p>
          <h1 className="mt-4 text-4xl font-semibold tracking-[-0.055em] text-pmri-text md:text-6xl">
          ...
          <div className="mt-8 grid gap-3 text-sm text-pmri-text2 md:grid-cols-3">
            {["Email first", "Verify code", "Then onboarding"].map((item, index) => (
              <div key={item} className="rounded-2xl border border-pmri-border/55 bg-white/[0.025] px-4 py-3">
                <span className="data-figure text-pmri-blueSoft">0{index + 1}</span>
        ...
        <div className="pmri-card rounded-[2rem] p-6 shadow-decision">
          {status === "signed_in" ? (
            ...
            <form onSubmit={submitEmail}>
              ...
              <label className="mt-6 block">
                ...
                <input
                  ...
                  className="pmri-focus mt-2 w-full rounded-2xl border border-pmri-border/55 bg-pmri-secondary/85 px-4 py-3 text-base text-pmri-text placeholder:text-pmri-muted/60"
                />
              </label>
              ...
              {isLocalhost ? (
                <button
                  ...
                  className="pmri-focus mt-3 w-full rounded-full border border-pmri-border/60 px-5 py-3 text-sm font-semibold text-pmri-text2 transition hover:border-pmri-border hover:bg-white/[0.04]"
                >
            ...
            <form onSubmit={submitCode}>
              <p className="pmri-label text-pmri-blueSoft">Step 02 / Verification</p>
              <h2 className="mt-3 text-2xl font-semibold tracking-[-0.035em] text-pmri-text">Verify your code</h2>
              ...
              <label className="mt-6 block">
                ...
                <input
                  ...
                  placeholder="123456"
                  className="pmri-focus mt-2 w-full rounded-2xl border border-pmri-border/55 bg-pmri-secondary/85 px-4 py-3 text-center text-2xl font-semibold tracking-[0.3em] text-pmri-text placeholder:text-pmri-muted/45"
                />
              </label>
