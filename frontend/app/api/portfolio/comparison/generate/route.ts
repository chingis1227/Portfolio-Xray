...
function validateRequest(body: ComparisonRequest) {
...
export async function POST(request: Request) {
  ...
  if (errors.length) {
    return jsonError("Comparison request validation failed.", 400, errors);
  }
  ...
  try {
    ...
    console.error("Failed to start comparison bridge.", error);
    return jsonError("Could not start comparison.", 500);
  }

  console.info("Comparison bridge finished.", {
  ...
  if (!resultPath) {
    return jsonError("Comparison did not return a result path.", 500, [
  ...
  try {
    ...
    console.error("Failed to read current_vs_candidate_result.json.", error);
    ...
    return NextResponse.json(
      {
        ...
        error: typeof resultObject.error === "string" ? scrubForClient(resultObject.error, root) : "Comparison failed.",
