...
function validateRequest(body: VerdictRequest) {
...
export async function POST(request: Request) {
  ...
  if (errors.length) {
    return jsonError("Decision verdict request validation failed.", 400, errors);
  }
  ...
  try {
    ...
    console.error("Failed to start decision verdict bridge.", error);
    return jsonError("Could not start decision verdict.", 500);
  }

  console.info("Decision verdict bridge finished.", {
  ...
  if (!resultPath) {
    return jsonError("Decision verdict did not return a result path.", 500, [
  ...
  try {
    ...
    console.error("Failed to read decision_verdict_result.json.", error);
    ...
    return NextResponse.json(
      {
        ...
        error: typeof resultObject.error === "string" ? scrubForClient(resultObject.error, root) : "Decision verdict failed.",
