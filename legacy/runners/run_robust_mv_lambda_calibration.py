...
def main() -> None:
    setup_logging()
    args = parse_args()
    config_path = Path(args.config).resolve() if args.config else REPO_ROOT / "config.yml"
    ...
    root = Path(args.output_dir).resolve() if args.output_dir else REPO_ROOT / "analysis_robust_mv_lambda_calibration"
