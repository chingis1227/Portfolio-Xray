...
"""
Build Robust Mean–Variance (constrained) baseline: same box bounds as constrained MinVar / MaxDiv.

James–Stein shrunk mu; Ledoit–Wolf or OAS shrunk Sigma; Young per-ticker caps when dual policy enabled.

Outputs under ``robust mean variance constrained portfolio/``.
"""
...
def main() -> None:
    ...
    result = build_robust_mean_variance_constrained(
        cfg,
        monthly_returns,
        analysis_end_str,
        primary_window,
    ...
    out_dir = REPO_ROOT / "robust mean variance constrained portfolio"
    ...
    try:
        ...
        logger.warning("Could not compute RC_vol for Robust MV constrained baseline: %s", e)
        rc_series = None
    ...
    if result.status not in ("OK", "APPROXIMATE"):
        ...
        with open(out_dir / "summary.txt", "w", encoding="utf-8") as f:
            f.write(f"{BASELINE_ROBUST_MV_CONSTRAINED_LABEL} — infeasible or failed baseline\n")
            f.write(f"Status: {result.status}\n")
            if summary.get("reason"):
                f.write(f"Reason: {summary['reason']}\n")
        print("Robust Mean–Variance (constrained) baseline failed or infeasible; summary written.")
        return
    ...
    with open(out_dir / "summary.txt", "w", encoding="utf-8") as f:
        f.write(f"{BASELINE_ROBUST_MV_CONSTRAINED_LABEL}\n")
        f.write("=" * 50 + "\n\n")
        if pm_summary:
            f.write(
        ...
        md = meta_export or {}
        f.write(
            f"\nOptimizer: {md.get('optimizer_name', 'robust_mean_variance_constrained')} "
    ...
    print(f"Robust Mean–Variance (constrained) baseline report written to {out_dir}")
