...
"""
Build Risk Budget by asset-class baseline (SLSQP on aggregated RC to variance) and full report.

Policy note: no policy optimizer, RC caps, or mandate overlays—benchmark only.
"""
...
def main() -> None:
    ...
    rb_result = build_risk_budget_by_asset_class_baseline(
        cfg,
        monthly_returns,
        analysis_end_str,
        primary_window,
    ...
    out_dir = Path(__file__).resolve().parent / "risk budget by asset-class portfolio"
    ...
    try:
        ...
        logger.warning("Could not compute RC_vol for risk budget (class) baseline: %s", e)

    export_baseline_weights_txt(
        rb_result.weights, rc_series=rc_series, label=BASELINE_RISK_BUDGET_BY_ASSET_CLASS_LABEL, output_dir=out_dir
    )

    if rb_result.status not in ("OK", "APPROXIMATE"):
        summary = {
            "portfolio_type": BASELINE_RISK_BUDGET_BY_ASSET_CLASS_LABEL,
        ...
        with open(out_dir / "summary.txt", "w", encoding="utf-8") as f:
            f.write(f"{BASELINE_RISK_BUDGET_BY_ASSET_CLASS_LABEL} — infeasible baseline\n")
            f.write(f"Status: {rb_result.status}\n")
            if summary.get("reason"):
                f.write(f"Reason: {summary['reason']}\n")
        logger.error("Risk budget (class) baseline failed: %s", rb_result.status)
        raise SystemExit(1)
    ...
    summary = {
        "portfolio_type": BASELINE_RISK_BUDGET_BY_ASSET_CLASS_LABEL,
        "status": rb_result.status,
        "solver_status": rb_result.diagnostics.get("solver_status"),
    ...
    with open(out_dir / "summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
    with open(out_dir / "summary.txt", "w", encoding="utf-8") as f:
        f.write(f"{BASELINE_RISK_BUDGET_BY_ASSET_CLASS_LABEL}\n")
        f.write("=" * 50 + "\n\n")
        if pm_summary:
            f.write(
        ...
        f.write(
            f"Risk budget tracking error: {rb_result.diagnostics.get('risk_budget_tracking_error')}\n"
        )
        f.write(f"Max budget deviation: {rb_result.diagnostics.get('max_budget_deviation')}\n")

    logger.info("Risk budget (class) baseline report written to %s", out_dir)
