...
- `stress_report.json.macro_regime_diagnostics` follows [docs/docs/stress_testing_spec.md](docs/docs/stress_testing_spec.md): it adds diagnostic-only `internal_market_proxy_v1` regime labels, latest growth/inflation-pressure scores, confidence and transition warnings, regime-specific factor betas/covariance/RC with `base_10y` fallback metadata, and CSV exports `macro_regime_labels_weekly.csv`, `macro_regime_factor_betas.csv`, `macro_regime_factor_covariance.csv`, and `macro_regime_factor_rc.csv`.
