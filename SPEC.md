...
| Current Portfolio Stress Scorecard (Block 3.4) | Product contract `current_portfolio_stress_scorecard_v1` on `stress_report.json` — diagnostic-only summary adapter over Blocks 3.1–3.3 (`stress_results_v1` + `hedge_gap_analysis_v1`); legacy `stress_scorecard_v1` retained for compatibility |
