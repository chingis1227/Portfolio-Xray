...
- `stress_report.json.regime_factor_analytics` (`regime_factor_analytics_v1`) follows [docs/docs/stress_testing_spec.md](docs/docs/stress_testing_spec.md) §8.8.3: diagnostic-only per-primary-regime monthly asset/factor covariance (Ledoit–Wolf on complete-case rows with documented fallbacks) and correlation, nine-factor asset betas with HAC inference, bottom-up portfolio factor exposure, factor variance contribution, and optional weekly alignment; it exports `regime_*.csv` under `results_csv/` and `regime_factor_analytics_summary.json` under `output_dir_final`, does not embed full covariance matrices in the slim JSON block, and does not affect optimization or stress gates.
