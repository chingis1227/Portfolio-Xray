...
2. **РћС‚С‡С‘С‚**
  ...
  Macro regime diagnostics in `stress_report.json.macro_regime_diagnostics` use `macro_two_axis_v1`: a monthly two-axis macro classifier that produces a `growth_score` from five blocks (business activity, labor, consumer, credit, growth nowcast) and an `inflation_score` from five blocks (core inflation, headline inflation, wages, inflation expectations, business price pressure). Indicators flow through `src/data_macro_sources.py` (FRED → Yahoo → official CSV → official API → keyed API → manual CSV); the GDPNow nowcast is loaded from FRED `GDPNOW` (quarterly, ffilled to monthly). Series are normalised by rolling 10-year monthly z-scores, bucketed at `±0.5`, blended `0.6·momentum + 0.4·level`, and labelled (with a 1-month publication lag) as `goldilocks`, `reflation`, `stagflation`, `recession_disinflation`, or `neutral_transition`. The block reports `coverage_tier` and `confidence_level`, lists `available_blocks`, `missing_blocks`, and `optional_blocks_missing`, and runs per-regime monthly OLS / covariance / RC analytics gated by n_obs. It exports `macro_regime_labels_monthly.csv` (replaces the old weekly file), `macro_regime_factor_betas.csv`, `macro_regime_factor_covariance.csv`, `macro_regime_factor_rc.csv`, and the new `macro_regime_indicator_panel.csv`. It also includes a diagnostic-only `regime_label_quality_check` summary and exports `regime_label_quality_summary.json`, `regime_label_quality_by_regime.csv`, `regime_label_episode_history.csv`, and `regime_label_stability_summary.csv`. It is diagnostic-only and does not change optimizer weights, mandate gates, stress pass/fail, or weight release.
  `stress_report.json.regime_factor_analytics` (`regime_factor_analytics_v1`) adds diagnostic-only per-primary-regime **weekly** asset/factor covariance (Ledoit–Wolf), nine-factor asset betas with HAC inference, bottom-up portfolio factor exposure, and factor variance contribution, with **monthly** macro labels forward-filled to weeks (monthly fallback if weekly series cannot be built). **Portfolio-facing** regime stats are limited to the standard **10Y** window ending at `analysis_end`, with JSON/CSV fields that separate full label history (`regime_label_history_span`) from that overlap (`portfolio_regime_analytics_window`). It writes `regime_*.csv` tables under `results_csv/` and `regime_factor_analytics_summary.json` under `output_dir_final`; the JSON block in `stress_report.json` is slim (no full covariance matrices). It does not feed the optimizer or gates in v1.
