...
2. **РћС‚С‡С‘С‚**
  ...
  `stress_report.json.regime_factor_analytics` (`regime_factor_analytics_v1`) adds diagnostic-only per-primary-regime monthly asset/factor covariance, nine-factor asset betas with HAC inference, bottom-up portfolio factor exposure, and factor variance contribution, aligned to lagged macro labels. It writes `regime_*.csv` tables under `results_csv/` and `regime_factor_analytics_summary.json` under `output_dir_final`; the JSON block in `stress_report.json` is slim (no full covariance matrices). It does not feed the optimizer or gates in v1.
