...
"""
Build Maximum-Diversification baseline portfolio and run full metrics / stress pipeline.
...
Maximum diversification: same eligible-universe filter and monthly **Σ** path as constrained
Minimum Variance; maximizes diversification ratio ``(sigma' w) / sqrt(w' Sigma w)`` with PSD-repaired **Σ**, **SLSQP** with analytic Jacobian, ``sum(w) = 1``, and box bounds via
:class:`src.optimization._build_bounds`.

This script is **maximum_diversification_constrained** only. Outputs live under ``maximum diversification portfolio/``.
"""
...
def main() -> None:
    ...
    md_result = build_maximum_diversification_constrained(
        cfg,
        monthly_returns,
        analysis_end_str,
        primary_window,
    ...
    out_dir = Path(__file__).resolve().parent / "maximum diversification portfolio"
    ...
    try:
        ...
        logger.warning("Could not compute RC_vol for Maximum-Diversification baseline: %s", e)
        rc_series = None

    export_baseline_weights_txt(
        md_result.weights, rc_series=rc_series, label=BASELINE_MD_LABEL, output_dir=out_dir
    )

    if md_result.status not in ("OK", "APPROXIMATE"):
        summary = {
            "portfolio_type": BASELINE_MD_LABEL,
        ...
        with open(out_dir / "summary.txt", "w", encoding="utf-8") as f:
            f.write(f"{BASELINE_MD_LABEL} — infeasible or failed baseline\n")
            f.write(f"Status: {md_result.status}\n")
            if summary.get("reason"):
                f.write(f"Reason: {summary['reason']}\n")
        print("Maximum-Diversification baseline failed or infeasible; summary written.")
        return
    ...
    with open(out_dir / "summary.txt", "w", encoding="utf-8") as f:
        ...
        f.write(
            f"\nOptimizer: {meta_diag.get('optimizer_name', 'maximum_diversification_constrained')} "
    ...
    print(f"Maximum-Diversification baseline report written to {out_dir}")
