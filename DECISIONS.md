...
- Decision: The full factor-matrix path uses a separate bounded live FRED fetch budget for raw
  factor series, then either uses approved raw factor cache or fails clearly with the missing series
  named. The product diagnostic path must not switch to equity-only factor evidence as a silent demo
  shortcut.
- Context: Full Demo MVP readiness Session 05 found that an unavailable FRED endpoint could still
  spend roughly 31 seconds per raw factor series and about 282 seconds for a full warm-up attempt
  before failing.
- Rationale: A demo-safe system should be predictable: valid cache can preserve full factor evidence,
  but absent/invalid cache should be an explicit fast blocker rather than a multi-minute hang.
- Consequences: Factor cache policy metadata now includes the bounded live-fetch budget; operators
  can run `scripts/warm_factor_cache.py` before demos; FRED CSV calls request only the needed date
  range.
- Date/Author: 2026-06-05 / Codex.

### DEC-2026-06-05-007 - Product-run metadata gates stale vertical artifacts

- Decision: Blocks 5-9 vertical product artifacts may carry explicit `product_run` metadata. When present, candidate generation, comparison, verdict, and AI Commentary grounding must share the same run id; diagnosis-only tombstones and inactive artifacts are not authoritative comparison inputs.
- Context: Full Demo MVP readiness Session 04 found that tests and operators need a machine-readable way to distinguish fresh vertical artifacts from historical JSON left in the same output directory.
- Rationale: Historical artifacts are useful and should not be broadly deleted, but product-facing readers must not accidentally treat an old candidate, comparison, or verdict as belonging to the current run.
- Consequences: Block 8 rejects tombstoned/not-authoritative/inactive `candidate_generation.json`; AI Commentary ignores a `decision_verdict.json` whose explicit `product_run.run_id` differs from `current_vs_candidate.json` and emits an `artifact_lineage_mismatch:*` warning.
- Date/Author: 2026-06-05 / Codex.


Decision ID: DEC-2026-06-05-006
Title: Product docs define the Blocks 5-9 wording boundary

- Status: accepted
- Date: 2026-06-05
- Decision: Make the documentation consistently describe Blocks 5-9 as Builder setup -> one candidate attempt -> current-vs-candidate comparison -> Decision Verdict, with explicit wording that Builder setup is not a candidate, a candidate is not a recommendation, reference tests are diagnostic comparisons, and Decision Verdict is where action/no-action is evaluated.
- Context: Session 09 of the Blocks 5-9 vertical loop is documentation sync after the one-command demo was implemented.
- Rationale: The product should feel like a guided decision-support workflow, not an optimizer cockpit or recommendation engine. Consistent docs prevent operators from treating setup files, reference benchmarks, or candidate artifacts as automatic rebalance advice.
- Alternatives considered: Leave prior one-candidate docs centered on `run_portfolio_review.py --candidates` (rejected because it skips the Builder/Block 7 artifact loop); describe Equal Weight reference tests as recommendations (rejected because they are diagnostic comparisons); hide no-trade/evidence-insufficient as failures (rejected because they are valid verdicts).
- Assumptions: `candidate_generation_v1`, `current_vs_candidate_v1`, `decision_verdict_v1`, and `ai_commentary_context_v1` remain the current additive contracts.
- Consequences: README, runtime entrypoints, operator guide, product/architecture docs, specs index, output docs, and plan registers must keep this wording unless a future spec intentionally changes the product boundary.
- Related documents: [README.md](README.md), [runtime_entrypoints.md](docs/runtime_entrypoints.md), [product_flow_operator_guide.md](docs/product_flow_operator_guide.md), [OUTPUTS.md](OUTPUTS.md), [docs/specs/README.md](docs/specs/README.md), [Blocks 5-9 Vertical Product Loop ExecPlan](docs/exec_plans/2026-06-05_blocks_5_9_vertical_product_loop_plan.md).
- Review trigger: Revisit when a UI changes the Builder/Candidate/Decision boundary or if a future schema makes generated candidates recommendations.

Decision ID: DEC-2026-06-05-005
Title: Blocks 5-9 demo uses one selected Launchpad hypothesis by default

- Status: accepted
- Date: 2026-06-05
- Decision: Use `scripts/run_blocks_5_to_9_vertical_flow.py` as the one-command vertical demo for Blocks 5-9. The command runs diagnosis-only review first, selects one Launchpad card, rebuilds one Builder setup, generates one candidate attempt, scopes comparison to that candidate, then writes direct Decision Verdict and AI Commentary grounding.
- Context: Blocks 5-9 Session 08 needed a demo path that proves `Diagnosis -> Hypothesis -> Candidate -> Comparison -> Verdict` without turning the product into a candidate zoo or relying on the advanced Selection Engine path.
- Rationale: A single selected hypothesis keeps the current product diagnosis-first and decision-support-first. Prefering reference or mixed-evidence cards and Equal Weight by default gives a transparent first demo while preserving targeted-card overrides.
- Alternatives considered: Use `run_portfolio_review.py --with-candidates` or `--mode full` as the demo (rejected because those are backend/research batches); run `run_compare_variants.py` all-in-one after candidate generation (rejected because it refreshes advanced downstream artifacts outside the Block 8 boundary); leave Session 08 as separate manual commands (rejected because it does not prove the vertical loop).
- Assumptions: Block 6 Builder setup remains non-recommendational, Block 7 remains one attempt only, and Decision Verdict remains the first action/no-action layer.
- Consequences: The vertical demo clears stale root Block 7-9 artifacts before candidate generation, defaults to `equal_weight`, and writes only the product bundle chain plus scoped technical comparison evidence.
- Related documents: [runtime_entrypoints.md](docs/runtime_entrypoints.md), [OUTPUTS.md](OUTPUTS.md), [Blocks 5-9 Vertical Product Loop ExecPlan](docs/exec_plans/2026-06-05_blocks_5_9_vertical_product_loop_plan.md), [scripts/run_blocks_5_to_9_vertical_flow.py](scripts/run_blocks_5_to_9_vertical_flow.py).
- Review trigger: Revisit if the product UI introduces explicit card selection before demo execution or if `candidate_generation_v2` changes the one-attempt handoff.

Decision ID: DEC-2026-06-05-004
Title: AI Commentary grounding can cite Block 7 vertical-loop evidence

- Status: accepted
- Date: 2026-06-05
- Decision: Allow `ai_commentary_context.json` to cite `candidate_generation.json` alongside `current_vs_candidate.json` and `decision_verdict.json` for the Blocks 5-9 vertical product loop, while keeping the layer deterministic and grounding-only.
- Context: Blocks 5-9 Session 07 needed future AI Commentary to explain the diagnosis, tested hypothesis, generated candidate, trade-offs, success criteria, verdict, no-trade rationale, and monitoring trigger without relying on the advanced Selection Engine path.
- Rationale: Block 7 preserves the hypothesis and candidate setup evidence that Block 8 and Block 9 use. Treating it as an allowed source keeps explanations grounded in the actual one-candidate path and avoids inventing hypotheses or recommendations.
- Alternatives considered: Keep AI grounding limited to `selection_decision.json` and `candidate_comparison.json` (rejected because the direct vertical loop may not have Selection evidence); generate natural-language AI commentary now (rejected because the approved current boundary is deterministic grounding only); let AI infer the candidate hypothesis from comparison rows (rejected because it would be unsupported).
- Assumptions: `candidate_generation.json` remains diagnostic-only and keeps `is_rebalance_recommendation: false`; Block 8 and Block 9 continue to own comparison and verdict semantics.
- Consequences: `src/ai_commentary_context.py` accepts `candidate_generation`, records direct Block 7/8/9 evidence references, and does not warn about missing `selection_decision.json` or `candidate_comparison.json` when the direct path has `candidate_generation.json`, `current_vs_candidate.json`, and `decision_verdict.json`.
- Related documents: [ai_commentary_grounding_spec.md](docs/specs/ai_commentary_grounding_spec.md), [candidate_generation_spec.md](docs/specs/candidate_generation_spec.md), [decision_verdict_spec.md](docs/specs/decision_verdict_spec.md), [Blocks 5-9 Vertical Product Loop ExecPlan](docs/exec_plans/2026-06-05_blocks_5_9_vertical_product_loop_plan.md).
- Review trigger: Revisit when natural-language AI Commentary generation is specified or if `candidate_generation_v2` changes the Block 7 evidence contract.

Decision ID: DEC-2026-06-05-003
Title: Block 9 can build verdicts directly from Block 7 and Block 8 evidence

- Status: accepted
- Date: 2026-06-05
- Decision: Keep `decision_verdict_v1` and its existing verdict/status vocabulary, but allow the vertical product loop to build `decision_verdict.json` directly from one `candidate_generation.json` attempt and one `current_vs_candidate.json` comparison. Add product-specific `verdict_reason_id`, `reviewed_candidate_id`, and `evidence_summary` fields instead of renaming Selection Engine schemas or inventing a separate verdict artifact.
- Context: Blocks 5-9 Session 06 needed Decision Verdict to evaluate one generated hypothesis without depending on advanced Selection Engine ranking or a candidate zoo.
- Rationale: The current product is diagnosis-first decision support. Direct Block 7/8 evidence is enough to say evidence insufficient, no-trade/keep-current, test another candidate, risk improved but turnover too high, or material enough for rebalance review. Reusing the existing verdict ids keeps validators, AI grounding, and monitoring compatible.
- Alternatives considered: Run the advanced Selection Engine for the vertical demo (rejected because it reintroduces ranking/scorecard behavior into the MVP path); create a new `decision_verdict_v2` artifact (rejected because Session 06 only needs an additive direct builder); issue a rebalance verdict whenever risk improves (rejected because high turnover/cost and failed success criteria must be valid no-trade reasons).
- Assumptions: Block 7 still creates exactly one non-recommendational candidate attempt; Block 8 still exposes trade-offs, materiality, success criteria, and practicality without creating a verdict.
- Consequences: `src/decision_verdict.py` now has `build_decision_verdict_from_block7_8()`; direct verdict output may include `source_artifacts.selection_decision: null`; no-trade and evidence-insufficient are first-class successful outcomes, not runtime failures.
- Related documents: [decision_verdict_spec.md](docs/specs/decision_verdict_spec.md), [candidate_generation_spec.md](docs/specs/candidate_generation_spec.md), [current_vs_candidate_spec.md](docs/specs/current_vs_candidate_spec.md), [Blocks 5-9 Vertical Product Loop ExecPlan](docs/exec_plans/2026-06-05_blocks_5_9_vertical_product_loop_plan.md).
- Review trigger: Revisit if `decision_verdict_v2` is intentionally introduced, if a UI needs separate action labels, or if transaction-cost/turnover guardrails become formal policy thresholds.

Decision ID: DEC-2026-06-05-002
Title: Guided Block 6 exposes only the MVP method set

- Status: accepted
- Date: 2026-06-05
- Decision: Limit the guided Portfolio Alternatives Builder path to Equal Weight, Risk Parity, Hierarchical Risk Parity, Minimum Variance, Minimum CVaR, and Maximum Diversification. Support only `capped` and `uncapped` modes; expose only `min_asset_weight` and `max_asset_weight` as user-adjustable optimization fields; keep uncapped mode diagnostic-only with an explicit concentration warning.
- Context: The Blocks 5-9 vertical product loop needs one clear hypothesis setup, not an optimizer cockpit or candidate zoo.
- Rationale: The MVP methods cover simple references, diversification/concentration tests, and downside/tail-risk tests while keeping the user focused on diagnosis -> hypothesis -> candidate -> comparison -> verdict.
- Alternatives considered: Keep Equal Weight by Asset Class, risk-budget methods, Robust Mean-Variance, Robust Scenario Optimization, Minimum Variance Advanced Controls, and Legacy Policy Optimizer in the guided menu (rejected because they make the MVP feel optimizer-first and expose advanced controls too early); remove those backend methods entirely (rejected because they remain useful advanced/legacy evidence).
- Assumptions: Hidden methods remain callable through explicit backend/legacy paths where already supported; Block 7 will consume the guided `CandidateSetup` and map capped/uncapped variants to backend candidate ids.
- Consequences: Launchpad/Builder guided method validation uses the MVP allowlist; Minimum CVaR is the product method id and maps to constrained or uncapped backend engines by mode; uncapped setup carries the concentration warning and is not a rebalance recommendation.
- Related documents: [portfolio_alternatives_builder_spec.md](docs/specs/portfolio_alternatives_builder_spec.md), [candidate_setup_spec.md](docs/specs/candidate_setup_spec.md), [candidate_launchpad_spec.md](docs/specs/candidate_launchpad_spec.md), [Blocks 5-9 Vertical Product Loop ExecPlan](docs/exec_plans/2026-06-05_blocks_5_9_vertical_product_loop_plan.md).
- Review trigger: Revisit when the vertical demo is accepted or when a product spec promotes an advanced optimizer into the guided MVP path.

Decision ID: DEC-2026-06-05-001
Title: Block 6 emits validated Builder setup, not candidate portfolios

- Status: accepted
- Date: 2026-06-05
- Decision: Treat `portfolio_alternatives_builder.json` as the Block 6 product setup artifact. It may contain `BuilderPrefill`, validation, and a validated `CandidateSetup` handoff, but it must not contain candidate ids, weights, portfolio metrics, stress results, comparison output, or a verdict. Candidate Generation remains Block 7 and requires an explicit Generate Candidate action.
- Context: Block 6 Sessions 06-09 added runtime Builder artifact writing after Launchpad and synchronized documentation. The old one-candidate delegation helper still exists, but it contains factory command/candidate id state and is not the canonical Block 6 output.
- Rationale: Users need a clean, inspectable setup boundary between choosing a Launchpad card and spending time or trust on generated candidate portfolios. Separating setup from generation prevents data-quality cards and reference benchmarks from being mistaken for rebalance recommendations.
- Alternatives considered: Keep Builder as in-memory prefill only (rejected because UI/API consumers need the product-bundle artifact); expose `PortfolioAlternativeBuildPlan` as the main Block 6 output (rejected because it includes candidate-generation plumbing); auto-generate candidates after Launchpad (rejected because it removes explicit user consent and blurs Block 6/Block 7).
- Assumptions: `run_portfolio_review.py` remains diagnosis-only by default; explicit `--candidates` and candidate factory commands remain preserved Block 7/backend paths.
- Consequences: Docs and product-bundle discovery include `portfolio_alternatives_builder_json`; data-quality Builder documents are blocked with `candidate_setup: null`; valid targeted/reference setup can set `can_generate_candidate: true` without generating anything.
- Related documents: [portfolio_alternatives_builder_spec.md](docs/specs/portfolio_alternatives_builder_spec.md), [builder_prefill_spec.md](docs/specs/builder_prefill_spec.md), [candidate_setup_spec.md](docs/specs/candidate_setup_spec.md), [Block 6 Builder ExecPlan](docs/exec_plans/block_6_builder_exec_plan.md).
- Review trigger: Revisit when Block 7 consumes `CandidateSetup` directly or when a persistent Builder UI/workspace schema is introduced.
...
- Alternatives considered: Keep v2 schema and add more score weights (rejected вЂ” increases opacity); keep conflict as a normal primary verdict (rejected вЂ” sounds like the system failed); preserve v2 as a current legacy product path (rejected вЂ” current product contract should be unambiguous).
...
- Status: accepted (implemented вЂ” Session 14 closure 2026-05-29)
- Date: 2026-05-29
- Decision: Upgrade Block 4 entry from thin `problem_classification_v1` to structured `Block 4 v2 problem-classification schema` (evidence extraction в†’ scoring в†’ severity/confidence в†’ prioritization в†’ actions в†’ launchpad) via additive schema bump on the same filenames (`analysis_subject/problem_classification.json`, `candidate_launchpad.json`). During Sessions 10вЂ“13, dual validation ran; compatibility shim (`problems[]` mirror, severity `medium` в†’ `moderate`) retained. **Session 14:** V1 product validators removed; that contract was canonical for live E2E and decision-entry tests until DEC-2026-06-04-001 superseded it; legacy V1 builders remain for unit tests only. Block 4 remains read-only over Blocks 2вЂ“3.
...
- Alternatives considered: In-place V1 rewrite without schema_version bump (rejected вЂ” silent contract drift); separate bundle files (rejected вЂ” six-file product bundle unchanged); big-bang cutover without shim (rejected вЂ” breaks Session 09 E2E gates).
- Assumptions: Blocks 2.1вЂ“2.6 and 3.3вЂ“3.4 product blocks remain stable evidence sources; core-only hygiene continues to omit Block 4 artifacts.
...
- Review trigger: Closed Session 14 вЂ” superseded by DEC-2026-06-04-001.
...
- Context: Phase D Session 10 after Session 09 Block 4 closure; Block 5 builders existed but lacked the same institutional contract + live profile gates as Blocks 3вЂ“4.
- Rationale: One-candidate product path must expose authoritative root compare/verdict JSON with fail-loud drift detection before Sessions 11вЂ“12 extend decision-package grounding.
- Alternatives considered: Validate only in offline unit tests (rejected вЂ” no operator workspace proof); merge verdict into `candidate_comparison.json` only (rejected вЂ” separate product contracts per spec).
...
- Rationale: Decision Workflow UI and operators need the same вЂњfail loud on contract driftвЂќ standard as Blocks 1вЂ“3; offline tests alone are insufficient without live profile integration.
- Alternatives considered: Only document specs without validators (rejected вЂ” regression risk); merge Block 4 into a separate script (rejected вЂ” duplicate subject checks).
- Assumptions: `run_report.py` continues to write PC then Launchpad on nonвЂ“core-only materialize; core-only runs remain without Block 4 subject files.
...
Title: Blocks 1вЂ“3 diagnostic foundation ready for Decision Workflow (Phase A closure)

- Status: accepted
- Date: 2026-05-29
- Decision: After ExecPlan Phase A Sessions 01вЂ“06, the project treats Blocks 1вЂ“3 runtime artifact scope as **frozen for Decision Workflow entry**: verdict **`READY_FOR_DECISION_WORKFLOW`**; operators use [runtime_artifact_contract.md](docs/runtime_artifact_contract.md) and `scripts/verify_live_core_e2e.py` (auto-detect or `--profile`) after each canonical CLI. Pre-decision audit `NOT_READY_RUNTIME_CONTRACT_MISMATCH` is superseded by [foundation closure audit](docs/audits/2026-05-29_blocks_1_3_foundation_closure_audit.md).
- Context: Pre-decision audit (2026-05-29) blocked Decision Workflow on R2вЂ“R5 (stale compare, 19-row menu, core-only leakage, E2E false failures). Sessions 02вЂ“05 implemented fixes; Session 06 re-ran three CLIs and re-scored acceptance criteria 1вЂ“18.
- Rationale: Live evidence on demo portfolio plus **261** pytest passes demonstrates product-scoped comparison and hygiene; downstream Blocks 4вЂ“7 consumers can treat root compare JSON as authoritative when tombstone / scope metadata match the active CLI mode.
- Alternatives considered: Keep `NOT_READY` until R6/R7 polish (rejected вЂ” P2 UX/scoring gaps do not invalidate runtime contract); skip live re-run and rely on offline tests only (rejected вЂ” audit required operator workspace proof).
...
- Consequences: Phase D (Decision workflow Sessions 09вЂ“12) may proceed; ExecPlan Phase A marked complete; `docs/audits/README.md` active closure pointer updated.
...
- Context: Pre-decision audit R5 вЂ” validator assumed every workspace was a `core_fast` batch compare (`review_mode=core`, 19-row menu), so one-candidate and diagnosis-only workspaces failed after Sessions 02вЂ“04 fixes.
...
- Alternatives considered: Re-run only `core_fast` before validate (rejected вЂ” wrong product contract); drop comparison checks entirely (rejected вЂ” loses regression signal); separate scripts per mode (rejected вЂ” duplicated subject Block 1вЂ“3 checks).
...
Title: Core Blocks 1вЂ“3 runs prune stale Block 4+ subject and root compare JSON

- Status: accepted
- Date: 2026-05-29
- Decision: After successful `run_materialize_analysis_subject_report` with `core_diagnostics_only=True` (`product_bundle_scope=core_blocks_1_3`), call `apply_core_blocks_product_bundle_hygiene` to **delete** stale `analysis_subject/{problem_classification,candidate_launchpad,ai_commentary_context}.json` and all root post-compare/decision JSON (compare, verdict, factory, selection, advanced package). Core-only root artifacts remain **absent** (not tombstoned). Blocks 1вЂ“3 subject JSON (`portfolio_xray`, `stress_report`, snapshots, manifest) is untouched.
- Context: Pre-decision audit R4 вЂ” `run_core_diagnostics.py` refreshed Blocks 1вЂ“3 but left Block 4+ product JSON from prior full review looking authoritative on disk while manifest excluded those keys.
- Rationale: Operators and validators must not read stale Problem Classification / Launchpad / compare files after a core-only run; deletion matches runtime contract вЂњAbsentвЂќ for core mode root artifacts.
- Alternatives considered: Tombstone Block 4+ subject files (rejected вЂ” core mode contract is absence, not вЂњnot authoritativeвЂќ sentinel); rely on manifest only (rejected вЂ” files remained on disk); prune in a separate CLI step (rejected вЂ” hygiene must be automatic on materialize).
...
- Context: Pre-decision audit R3 вЂ” default `run_portfolio_review.py` left stale `decision_verdict.json` / `current_vs_candidate.json` from prior one-candidate runs, misleading operators and UI consumers.
...
- Alternatives considered: Silent delete only (rejected вЂ” UI may interpret missing files as вЂњnot yet runвЂќ); leave stale files (rejected вЂ” audit failure); tombstone only verdict without comparison file (rejected вЂ” comparison menu was the primary misread surface before Session 02 scoping).
...
- Context: Pre-decision foundation audit (2026-05-29) found one-candidate CLI runs left **19-row** `candidate_comparison.json` while product adapters (`current_vs_candidate`, `decision_verdict`) were already scoped вЂ” operators could misread the comparison menu as the product answer (gap R2).
...
- Alternatives considered: Filter rows only in UI adapters while leaving JSON full (rejected вЂ” stale file remains authoritative on disk); omit full registry entirely (rejected вЂ” research and debugging still need on-disk scan); tombstone-only without scoped rewrite (deferred to Session 03 for diagnosis-only stale compare).
...
- Related documents: [docs/specs/portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md) В§2.6.1, [docs/specs/block_2_6_weakness_map_ui_pareto_spec.md](docs/specs/block_2_6_weakness_map_ui_pareto_spec.md), [docs/specs/stress_lab_layer_spec.md](docs/specs/stress_lab_layer_spec.md), [docs/specs/stress_testing_spec.md](docs/specs/stress_testing_spec.md), [docs/exec_plans/2026-05-29_block_2_6_weakness_map_heuristic_v2_plan.md](docs/exec_plans/2026-05-29_block_2_6_weakness_map_heuristic_v2_plan.md) (**Completed**), [docs/audits/2026-05-29_block_2_6_weakness_map_heuristic_v2_acceptance_audit.md](docs/audits/2026-05-29_block_2_6_weakness_map_heuristic_v2_acceptance_audit.md).
...
Title: Block 3.3 institutional upgrade вЂ” v1-primary downstream and main-gap scoring v2
...
- Context: MVP Block 3.3 (2026-05-27) delivered contribution-based offset coverage but legacy taxonomy hedge block was often `not_applicable`, main-gap selection favored tiny zero-offset losses, and consumers still read `hedge_gap_status`. Phase 2 ExecPlan Sessions 02вЂ“10 closed implementation gaps before acceptance audit.
- Rationale: Institutional-grade hedge-gap diagnosis requires transparent scoring, cross-block confirmation without circular imports, and a single v1 read path for classification, comparison, commentary grounding, and Core MVP validation вЂ” without re-running stress or pre-labeling hedges.
- Alternatives considered: Migrate by overwriting legacy `hedge_gap_analysis` in place (rejected вЂ” breaks contract tests and taxonomy semantics); drive Block 3.3 from `portfolio_xray.json` (rejected вЂ” Stress Lab boundary); drop legacy block in this wave (deferred вЂ” compatibility for scorecard rollups and older commentary paths).
...
Title: Block 3.4 institutional upgrade вЂ” v1-primary downstream and live-output gates
...
- Context: MVP Block 3.4 (DEC-2026-05-27-003) and frozen contract (DEC-2026-05-29-004) defined the key; Sessions 02вЂ“11 implemented builders and migrations; documentation and top-level SPEC/OUTPUTS still described Phase 2 as in progress until Session 12.
- Rationale: Institutional executive stress diagnosis requires a single v1 read path for classification, comparison, commentary grounding, and Core MVP validation вЂ” without recomputing stress or emitting mandate pass/fail inside Block 3.4.
- Alternatives considered: Repurpose `stress_scorecard_v1` as the only scorecard (rejected вЂ” mandate semantics); drop legacy scorecard in Phase 2 (deferred вЂ” mandate path); drive Block 3.4 from X-Ray only (rejected вЂ” Stress Lab boundary).
...
Title: Block 3.4 institutional upgrade вЂ” frozen v1.1 scorecard contract (Session 01)

- Status: accepted
- Date: 2026-05-29
- Decision: Freeze Phase 2 product contract for `current_portfolio_stress_scorecard_v1` under ruleset `current_portfolio_stress_scorecard_rules_v1_1` in [current_portfolio_stress_scorecard_spec.md](docs/specs/current_portfolio_stress_scorecard_spec.md): executive stress diagnosis read-only over Blocks 3.1вЂ“3.3; `stress_diagnosis.diagnosis_confidence` enum; `next_decision_uses[]`; `relatively_resilient_scenarios` / `less_damaging_scenarios` (no вЂњpasses normallyвЂќ); optional 2.4/2.6 `pre_stress_confirmation_summary` with `not_applicable` when unattached; Core MVP must not create internal mandate pass/fail (`legacy_fallback_used` boolean for explicit legacy scorecard use). Keep MVP field names one release alongside v1.1 aliases. Implementation Sessions 02вЂ“11 follow the spec status matrix.
...
- Alternatives considered: Extend `stress_lab_layer_spec.md` only without a dedicated spec (rejected вЂ” insufficient depth for v1.1 field rules); merge Block 3.4 into Block 3.2 envelope (rejected вЂ” product layer and downstream hooks are distinct); remove `stress_scorecard_v1` in Phase 2 (rejected вЂ” mandate path and explicit fallback required).
...
- Consequences: SPEC indexes dedicated spec; Phase 2 ExecPlan Session 01 closed; Sessions 02вЂ“11 implemented against the spec matrix (see DEC-2026-05-29-005); Session 12 synced SPEC/OUTPUTS/TESTING; acceptance at Session 13 requires live-output gates in the spec.
...
- Alternatives considered: Reuse proxy waterfall in Core MVP replay (rejected вЂ” false precision); hide historical episodes when coverage is partial (rejected вЂ” hides the limitation); replace `historical_results` entirely in `run_stress` (deferred вЂ” breaking change for legacy mandate and conclusions rollups).
...
- Related documents: [docs/specs/stress_lab_layer_spec.md](docs/specs/stress_lab_layer_spec.md) В§3.1.1, В§3.2, [docs/specs/stress_testing_spec.md](docs/specs/stress_testing_spec.md) В§9.4, [docs/exec_plans/2026-05-28_core_mvp_historical_stress_replay_plan.md](docs/exec_plans/2026-05-28_core_mvp_historical_stress_replay_plan.md), DEC-2026-05-20-001.
...
- Context: Product brief requires hedge-gap diagnosis from signed scenario asset contributions without re-running stress or labeling hedge assets. Legacy block is often `not_applicable` when no hedge `risk_role` labels exist. Stress Lab layer spec previously numbered hedge gap as В§3.5 and simulator as В§3.3.
...
- Alternatives considered: Extend legacy `hedge_gap_analysis` in place (rejected вЂ” taxonomy coupling and different risk-type ids); place Block 3.3 on `portfolio_xray.json` (rejected вЂ” Stress Lab boundary); include `recession_severe` as an eighth v1 row (initially rejected вЂ” product brief listed seven protection areas; **promoted 2026-05-27** as `recession_severe_protection` for fixture-matrix hedge-gap completeness without changing Stress Results architecture).
- Assumptions: Block 3.2 `stress_results_v1` remains the preferred read path for per-scenario `pnl_by_asset_pct`; Core MVP stays `loss_gate_mode="diagnostic"`; historical episodes stay out of v1 `by_risk_type[]` until a canonical episodeв†’risk map exists.
- Consequences: Specs, PRODUCT В§4.3.3, OUTPUTS, and TESTING reference `hedge_gap_analysis_v1` as the Core MVP hedge-gap contract. MVP ExecPlan Sessions 02вЂ“08 and institutional upgrade Sessions 02вЂ“10 (see DEC-2026-05-29-003) delivered implementation and v1-primary downstream migration; `stress_conclusions.hedge_gap_status` still mirrors **legacy** aggregate status only.
- Related documents: [docs/specs/hedge_gap_analysis_spec.md](docs/specs/hedge_gap_analysis_spec.md), [docs/specs/stress_lab_layer_spec.md](docs/specs/stress_lab_layer_spec.md), [docs/specs/stress_testing_spec.md](docs/specs/stress_testing_spec.md) В§12.2.2, [PRODUCT.md](PRODUCT.md) В§4.3.3, [docs/exec_plans/2026-05-27_block_3_3_hedge_gap_analysis_plan.md](docs/exec_plans/2026-05-27_block_3_3_hedge_gap_analysis_plan.md), [docs/exec_plans/2026-05-29_block_3_3_hedge_gap_institutional_upgrade_plan.md](docs/exec_plans/2026-05-29_block_3_3_hedge_gap_institutional_upgrade_plan.md), DEC-2026-05-29-003.
- Review trigger: Revisit when legacy `hedge_gap_analysis` / `hedge_gap_status` coupling can be deprecated (partially satisfied вЂ” Core MVP consumers migrated; legacy block retained).
...
- Status: accepted
- Date: 2026-05-27
- Decision: Implement Block 3.4 Core MVP Current Portfolio Stress Scorecard as a new top-level key on `stress_report.json`: `current_portfolio_stress_scorecard_v1`, built as a diagnostic-only adapter over Blocks 3.1вЂ“3.3 (`scenario_results` / `historical_results`, `stress_results_v1`, `hedge_gap_analysis_v1`).
...
- Alternatives considered: Repurpose `stress_scorecard_v1` as the Core MVP scorecard (rejected вЂ” breaks existing contract tests and would mix mandate-mode semantics into a diagnostic-only product layer).
...
- Alternatives considered: Add placeholder `unavailable` registry rows for every concept name (rejected вЂ” inflates menu without builders); ignore concept drift in docs only (rejected вЂ” G9); implement all concept optimizers in Session 11 (rejected вЂ” non-goals for Phase 14).
...
- Context: Block 3 governance gap G8 вЂ” X-Ray surfaces vol and crypto risks but stress suite has eight synthetic scenarios on the six-shock engine; adding scenarios without a factor/crypto channel plan risks double counting or equity-proxy misstatement.
...
- Alternatives considered: Implement `volatility_shock` now (deferred вЂ” see proposal Option A); map crypto to `equity_shock` proxy (rejected); implement both in Session 08 without spec (rejected вЂ” violates governance non-goals).
...
- Consequences: `stress_testing_spec.md` В§2.3 documents deferred ids; methodology map G8 closed as spec-only; implementation requires proposal В§5 checklist, WEAKNESS_SCENARIO_MAP alignment, and Sessions 10вЂ“11 integration if later accepted.
- Related documents: [docs/proposals/2026-05-20_crypto_vol_stress_scenarios_proposal.md](docs/proposals/2026-05-20_crypto_vol_stress_scenarios_proposal.md), [stress_testing_spec.md](docs/specs/stress_testing_spec.md) В§2.3, [portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md) В§2.7, [docs/exec_plans/2026-05-20_stress_lab_methodology_governance_plan.md](docs/exec_plans/2026-05-20_stress_lab_methodology_governance_plan.md) Session 08.
...
- Alternatives considered: Enable proxy in `run_stress` primary path (deferred вЂ” proposal P1); hide proxy entirely (rejected вЂ” misleads library readers).
...
- Consequences: `historical_methodology` block on stress reports; spec В§9.3; tests in `test_stress_historical_fields.py`.
- Related documents: [stress_testing_spec.md](docs/specs/stress_testing_spec.md) В§9, [scenario_library_spec.md](docs/specs/scenario_library_spec.md), [docs/exec_plans/2026-05-20_stress_lab_methodology_governance_plan.md](docs/exec_plans/2026-05-20_stress_lab_methodology_governance_plan.md) Session 02, [docs/audits/2026-05-20_stress_lab_methodology_map.md](docs/audits/2026-05-20_stress_lab_methodology_map.md).
...
Title: Selection Engine V1 contract вЂ” composite score, policy default, No-Trade materiality
...
- Context: Comparison and diagnostic scores (Sessions 08вЂ“13) exist; the product needs a formal non-executing decision artifact before Action Engine and Decision Journal.
...
- Alternatives considered: Auto-select top health rank only (rejected вЂ” ignores policy role and robustness); merge Selection into Health Score (rejected вЂ” blurs diagnostic vs decision); include Pareto pruning in V1 (deferred вЂ” no owning spec).
...
Title: Assumption Sensitivity V1 вЂ” selection-stability grid without optimizer re-run
...
- Alternatives considered: Full assumption grid with re-optimization (deferred вЂ” out of scope and high model risk); merge into model-risk artifact (rejected вЂ” different question); auto-downgrade selection when fragile (rejected вЂ” violates diagnostic boundary).
...
Title: Pareto / Dominance Check V1 вЂ” strict metric-only dominance diagnostic
...
- Alternatives considered: Mandate-aware dominance (rejected for V1 вЂ” blurs binding gates with metric evidence); auto-remove dominated candidates from Selection (rejected вЂ” violates diagnostic boundary); Sharpe-only dominance (rejected вЂ” overlaps Assumption Sensitivity Tier B).
...
Title: Regret Analysis V1 вЂ” stress-scenario opportunity-loss diagnostic
...
- Alternatives considered: Regret vs Pareto-efficient set only (deferred to V2); auto-switch selection on high regret (rejected вЂ” violates diagnostic boundary); full macro regime regret without projected comparison fields (deferred вЂ” optional slice with `not_available` when data missing).
...
Title: Trade-off Explanation and Model Risk Diagnostics V1 вЂ” separate diagnostic artifacts

- Status: accepted
- Date: 2026-05-17
- Decision: Adopt two file-first diagnostic artifacts under `output_dir_final`: `tradeoff_explanation_v1` (baseline `current` в†’ favored target from selection, metric/stress deltas without new formulas, weight-based turnover at write time) and `model_risk_diagnostics_v1` (deduplicated warning catalog from comparison, scores, stress, and run metadata). Pipeline placement is after `selection_decision.json` and before `action_plan.json`. Layer is non-binding and does not change selection, mandate, or stress pass/fail.
- Context: Post-audit Session 12 (RM-616 spec phase); audit PSA-012 and product concept sections 17вЂ“18 flagged partial coverage via selection bullets and scattered warnings only.
- Rationale: Makes вЂњprice of improvementвЂќ and model self-criticism explicit for decision package and journal without conflating them with Health Score or Selection outcomes.
- Alternatives considered: Single combined JSON (rejected вЂ” reporting and tests are clearer with two files); compute trade-off after action for action turnover (deferred вЂ” V1 uses weight deltas at trade-off write time); auto-veto selection on high model risk (rejected вЂ” violates diagnostic boundary).
...
- Decision: Product **Block 2.5 = Risk Budget View** (`block_2_5_risk_budget_view` on
  `portfolio_xray.json`). Core MVP product diagnosis extends to Blocks 2.1вЂ“2.6. Portfolio Archetype
  remains legacy `sections.portfolio_archetype` only (В§2.7; forbidden: `block_2_5_portfolio_archetype`).
...
- Rationale: Operators need a stable product surface for вЂњwho drives portfolio risk?вЂќ separate from
  capital allocation (2.1) and hidden-risk heuristics (2.4); Stress Test Lab owns scenario PnL.
- Alternatives considered: Keep risk budget as legacy-only В§2.6 (rejected вЂ” user-approved Core MVP 2.5);
  include stress PnL on product block (rejected вЂ” Stress Lab boundary); promote archetype as Block 2.5
  (rejected вЂ” postponed product module).
- Assumptions: Implementation lands Sessions 02вЂ“05; RC resolution stays in `build_portfolio_xray_v2`
  via `resolve_rc_asset_for_xray`; Block 2.5 module receives resolved rows only.
- Consequences: [portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md) В§2.5.1;
  planned `src/block_2_5_risk_budget_view.py`; [OUTPUTS.md](OUTPUTS.md) Block 2 row; [SPEC.md](SPEC.md) status matrix.
- Related documents: [portfolio_xray_layer_spec.md](docs/specs/portfolio_xray_layer_spec.md) В§2.5,
...
- Status: accepted
- Date: 2026-05-26
- Decision: Promote Portfolio Weakness Map as Core MVP **Block 2.6** (`block_2_6_portfolio_weakness_map` on `portfolio_xray.json`). The product block is a pre-stress hypothesis map: it aggregates signals from Blocks 2.1вЂ“2.5 into a 0вЂ“100 vulnerability score per risk type plus plain-English explanation and `next_tests` suggestions. It must **not** read `stress_report.json`, scenario PnL, pass/fail, or loss attribution. Legacy `sections.weakness_map` remains for backward compatibility and may remain stress-coupled.
- Context: A legacy Weakness Map section exists under `sections.weakness_map`, but it reads stress artifacts directly (scenario PnL and attribution) and exposes qualitative severity bands without a stable 0вЂ“100 scoring contract and explainable weights. Core MVP needs a product-facing вЂњwhat to test nextвЂќ surface that stays clearly separated from Stress Test Lab outcomes.
...
- Alternatives considered: Keep weakness map as legacy-only (rejected вЂ” requested product surface); compute scenario losses inside X-Ray (rejected вЂ” violates Stress Lab ownership); replace legacy section entirely (rejected вЂ” breaks compatibility and formatters).
- Assumptions: Block 2.6 can be implemented as a read-only adapter over Blocks 2.1вЂ“2.5 with explicit `heuristic_v1` rule weights and evidence rows; Stress Lab continues to own all loss numbers and pass/fail status.
- Consequences: `portfolio_xray_diagnostics_spec.md` adds В§2.6.1 contract; `portfolio_xray_layer_spec.md`, `SPEC.md`, `OUTPUTS.md`, and `GLOSSARY.md` align Core MVP as Blocks 2.1вЂ“2.6; Archetype remains legacy-only (В§2.7).
- Related documents: [Block 2.6 Weakness Map ExecPlan](docs/exec_plans/2026-05-26_block_2_6_portfolio_weakness_map_plan.md), [portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md) В§2.6.1, [stress_testing_spec.md](docs/specs/stress_testing_spec.md) (Stress Lab ownership).
...
- Alternatives considered: Recompute missing factor fields inside X-Ray (rejected вЂ” duplicate
  methodology and hidden fallback); embed stress-shock interpretation inside Block 2.3 (rejected вЂ”
  belongs to Stress Lab); standalone `factor_exposure.json` bundle artifact (rejected вЂ” Block 2
...
- Consequences: [portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md)
  В§2.3.1; implementation `src/block_2_3_factor_exposure.py`; tests
...
- Related documents: [factor_diagnostics_spec.md](docs/specs/factor_diagnostics_spec.md),
  [OUTPUTS.md](OUTPUTS.md) Block 2 row, [portfolio_xray_layer_spec.md](docs/specs/portfolio_xray_layer_spec.md) В§2.3.
...
- Alternatives considered: Embed metrics only in `sections.risk_diagnostics` (rejected вЂ” unstable for
  UI); standalone `portfolio_metrics.json` in six-file bundle (rejected); expose `metric_quality` to
  UI (rejected вЂ” internal diagnostics only).
...
- Consequences: [portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md)
  В§2.2.1; planned `src/block_2_2_portfolio_metrics.py`, `tests/test_block_2_2_portfolio_metrics.py`;
  acceptance audit `docs/audits/2026-05-26_block_2_2_portfolio_metrics_acceptance_audit.md` (Session 08).
- Related documents: [OUTPUTS.md](OUTPUTS.md) Block 2 row, [portfolio_xray_layer_spec.md](docs/specs/portfolio_xray_layer_spec.md) В§2.2.
...
- Alternatives considered: Standalone `asset_allocation.json` (rejected вЂ” extra manifest surface);
  replace `sections.asset_allocation` only (rejected вЂ” breaks golden contract tests and formatters).
- Assumptions: *(superseded 2026-05-26 Session 08)* вЂ” builder shipped; portfolio-first materialize
  paths populate the key; pre-2026-05-26 on-disk runs may lack Block 2.1 until re-materialized.
- Consequences: [portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md)
  В§2.1.1вЂ“В§2.1.2; drift tests `tests/test_block_2_1_threshold_registry.py`,
  ...
  ExecPlan Sessions 01вЂ“08 **Completed**.
- Related documents: [OUTPUTS.md](OUTPUTS.md) Block 2 row, [portfolio_xray_layer_spec.md](docs/specs/portfolio_xray_layer_spec.md) В§2.1.
...
- Status: accepted
- Date: 2026-05-26
- Decision: Freeze the Input Layer MVP contract after ExecPlan Sessions 01вЂ“10. Core MVP user input
...
- Rationale: Prevents endless first-screen churn; shifts active product work to Blocks 2вЂ“5 and
  product-bundle layers already wired behind portfolio-first review.
- Alternatives considered: Continue input UX iteration in parallel with X-Ray (rejected вЂ” splits
  focus); delete legacy config keys now (rejected вЂ” optimizer/research still need them under
...
- Consequences: Canonical behavior in [input_assumptions_spec.md](docs/specs/input_assumptions_spec.md)
  В§ Contract freeze; regression gate `tests/test_input_layer_mvp_regression.py`.
...
Title: Candidate Portfolio Factory V1 вЂ” orchestrate before compare
...
- Alternatives considered: Require manual script runs only (rejected вЂ” opaque and error-prone); embed all builders in one mega-optimizer (rejected вЂ” violates Main vs candidate boundary); auto-run policy inside factory (rejected вЂ” blurs production release path).
