...
- `problem_classification.json` (under each report output folder where `portfolio_xray.json` and `stress_report.json` are available; **v3 current** schema `problem_classification_v3` via `src/block_4/diagnosis_builder.py`; one primary diagnosis/outcome, root cause, supporting symptoms, max-five key evidence, why-not-other-problems, confidence/materiality/actionability, success criteria, and backend audit metadata; contracts in [block_4_diagnosis_v3_spec.md](docs/specs/block_4_diagnosis_v3_spec.md) and [problem classification spec](docs/specs/problem_classification_spec.md))
- `candidate_launchpad.json` (under each report output folder where Block 4 diagnosis is written; **v3 current** schema `candidate_launchpad_v3`; hypothesis cards with `source_diagnosis_id`, `hypothesis_to_test`, `suggested_methods`, `success_criteria`, trade-off/skip copy, disclaimer; no weights; contracts in [block_4_diagnosis_v3_spec.md](docs/specs/block_4_diagnosis_v3_spec.md) and [candidate launchpad spec](docs/specs/candidate_launchpad_spec.md))
