...
- `problem_classification.json` (under each report output folder where `portfolio_xray.json` and `stress_report.json` are available; diagnostic-only evidence translation from X-Ray/stress into top portfolio problems and reasonable paths to test; `hedge_gap_source` records v1-primary `hedge_gap_analysis_v1` vs legacy `stress_conclusions.hedge_gap_status` fallback; `stress_scorecard_source` records v1-primary `current_portfolio_stress_scorecard_v1` vs legacy `stress_scorecard_v1` fallback; contract in [problem classification spec](docs/specs/problem_classification_spec.md))
