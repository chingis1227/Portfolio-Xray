...
This file is the root map for generated outputs, report artifacts, output folders, and output-format ownership in Portfolio MRI.
...
| --- | --- | --- |
| `{output_dir_final}/analysis_subject/` | Portfolio-first **subject** diagnosed before candidates | Starting weights, Blocks 1-3 diagnostics (Diagnosis, stress, snapshots, `run_metadata.json`) for the reviewed portfolio |
...
2. Do **not** mix subject weights/diagnostics with root policy weights or root stress/Diagnosis files.
...
| --- | --- | --- |
| Portfolio-first subject materialization | Subject diagnostics before candidates | `{output_dir_final}/analysis_subject/` snapshots, metadata, Diagnosis, and diagnostics from `run_report.py --materialize-analysis-subject` |
...
| --- | --- | --- | --- |
| `client_fit_check.json` | `write_client_fit_check_outputs` in `run_report.py` (`src/client_fit.py`) | `{output_dir_final}/analysis_subject/client_fit_check.json` | After Stress Lab / Diagnosis and before Block 4; schema `client_fit_check_v1`; writes `not_provided` when no Client Fit profile exists (#1) |
...
- `portfolio_xray.json`
- `client_fit_check.json` (`client_fit_check_v1`; generated after Stress Lab/Diagnosis and before
...
| Portfolio Diagnosis JSON and seven-section diagnostic output | [docs/specs/portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md) |
