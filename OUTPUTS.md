...
| 2 X-Ray | `portfolio_xray.json` (seven sections + `block_2_1_asset_allocation` when Block 2.1 builder is wired + `block_2_2_portfolio_metrics` when Block 2.2 builder is wired) | `data_trust_signals.user_summary_lines` when data-quality warnings exist; prefer `block_2_1_asset_allocation` for capital structure UI (§2.1.1) and `block_2_2_portfolio_metrics` for portfolio behavior / risk metrics UI (§2.2.1 in [portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md)); legacy `sections.risk_diagnostics` remains for formatters until migration |
