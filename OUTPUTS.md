...
| --- | --- | --- | --- |
| `problem_classification.json` | `write_block_4_diagnosis_outputs` in `run_report.py` (`src/block_4/diagnosis_builder.py`) | `{output_dir_final}/analysis_subject/problem_classification.json` | After default diagnosis / materialize (#1); schema `problem_classification_v2` |
| `candidate_launchpad.json` | `write_block_4_diagnosis_outputs` in `run_report.py` | `{output_dir_final}/analysis_subject/candidate_launchpad.json` | After default diagnosis / materialize (#2); schema `candidate_launchpad_v2` |
...
- `problem_classification.json` (under each report output folder where `portfolio_xray.json` and `stress_report.json` are available; **v2 primary** schema `problem_classification_v2` via `src/block_4/diagnosis_builder.py`; one primary problem, secondaries, rejected hypotheses, no-trade gate, auditable `evidence_refs`; V1 shim `problems[]` for transitional readers; contracts in [block_4_diagnosis_v2_spec.md](docs/specs/block_4_diagnosis_v2_spec.md) and [problem classification spec](docs/specs/problem_classification_spec.md))
- `candidate_launchpad.json` (under each report output folder where Block 4 diagnosis is written; **v2 primary** schema `candidate_launchpad_v2`; hypothesis cards with `suggested_methods`, `default_method`, trade-off copy, disclaimer; no weights; contracts in [block_4_diagnosis_v2_spec.md](docs/specs/block_4_diagnosis_v2_spec.md) and [candidate launchpad spec](docs/specs/candidate_launchpad_spec.md))
