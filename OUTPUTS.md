...
| 2 X-Ray | `portfolio_xray.json` (seven sections + product blocks `block_2_1_asset_allocation`, `block_2_2_portfolio_metrics`, `block_2_3_factor_exposure`) | `data_trust_signals.user_summary_lines` when data-quality warnings exist; prefer product blocks for UI/API: capital structure (§2.1.1), portfolio behavior (§2.2.1), and factor sensitivity (§2.3.1 in [portfolio_xray_diagnostics_spec.md](docs/specs/portfolio_xray_diagnostics_spec.md)); Block 2.3 adapts existing `stress_report` factor diagnostics only and does not trigger calculations; legacy sections remain for formatters until migration |
