...
Стресс-тест: FAIL_STRESS (FAIL_ROLE_EQUITY_SHOCK); худший сценарий по убытку: equity_shock (Role); worst_scenario_loss_pct ≈ -22.71%.
...
Наибольшие доли RC_vol (вклад в дисперсию портфеля) на 10Y: COPX 11.4%, URA 9.3%, SMH 8.4%, ROBO 7.9%, ITA 6.3%. Стресс: status=FAIL_STRESS, fail_reason_code=FAIL_ROLE_EQUITY_SHOCK. Провал в сценарии «equity_shock», тест «Role».
...
Equal-Weight baseline: профиль доходности/риска на 10Y задаётся CAGR≈15.20% и vol≈12.98% при MaxDD≈-20.94%. Стресс FAIL_STRESS (FAIL_ROLE_EQUITY_SHOCK); клиентский gate PASS. Для сравнения вариантов используйте те же файлы в соседних папках (Equal-Weight / Risk Parity / Main portfolio) после синхронного прогона.
