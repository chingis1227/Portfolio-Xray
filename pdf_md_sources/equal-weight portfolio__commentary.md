...
Стресс-тест: FAIL_STRESS (FAIL_RC_TOP1_EQUITY_SHOCK); худший сценарий по убытку: equity_shock (RC_Top1); worst_scenario_loss_pct ≈ -178.61%.
...
Наибольшие доли RC_vol (вклад в дисперсию портфеля) на 10Y: COPX 11.4%, URA 9.3%, SMH 8.4%, ROBO 7.9%, ITA 6.3%. Стресс: status=FAIL_STRESS, fail_reason_code=FAIL_RC_TOP1_EQUITY_SHOCK. Провал в сценарии «equity_shock», тест «RC_Top1».
...
Стресс не пройден (FAIL_STRESS): FAIL_RC_TOP1_EQUITY_SHOCK. Именованный сценарий сбоя: equity_shock; тип проверки: RC_Top1.

## Scenario Behavior

Кратко по сценариям из stress_report.json: equity_shock: PnL≈-18.71%, pass=False; credit_shock: PnL≈-178.61%, pass=False; rates_shock: PnL≈-3.77%, pass=False; inflation_stagflation: PnL≈-8.61%, pass=False; liquidity_shock: PnL≈-142.15%, pass=False.
Худший сценарный убыток портфеля (worst_scenario_loss_pct): ≈ -178.61%.


## Final Conclusion

Equal-Weight baseline: профиль доходности/риска на 10Y задаётся CAGR≈15.20% и vol≈12.98% при MaxDD≈-20.94%. Стресс FAIL_STRESS (FAIL_RC_TOP1_EQUITY_SHOCK); клиентский gate PASS. Для сравнения вариантов используйте те же файлы в соседних папках (Equal-Weight / Risk Parity / Main portfolio) после синхронного прогона.
