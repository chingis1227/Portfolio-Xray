...
| `current_vs_candidate.json` | `src/current_vs_candidate.py` or Block 8 writer in `src/candidate_comparison.py`; root or active run-local folder after compare. | `/comparison`; `/verdict`; `/report`. | `reviewState.tsx` `recordComparisonResult`; `frontend/app/comparison/page.tsx`; comparison components; API `POST /api/portfolio/comparison/generate`. | Current-vs-candidate trade-off: what improved, worsened, stayed similar, is unavailable, how current/candidate values compare with Client Fit targets when available, and whether materiality supports decision review. | Missing metrics may be shown as fake conclusions; stale comparison can be trusted after a different candidate; raw `view_mode` / selected IDs can leak; Client Fit target rows can be mistaken for a verdict. | Only a current, scoped, same-candidate comparison unlocks Verdict. UI must separate no candidate, not comparable, partial metrics, complete comparison, target-reference rows, and stale/ignored states. Do not crown a winner or issue a verdict here. |
...
| `/client-fit` | Bounded FastAPI/adapter `client_fit` display summary derived from `client_fit_check.json`. | Client Profile input summary; Stress/X-Ray evidence already generated for the active review. | Raw `client_fit_check.json`, suitability approval, optimizer mandates, verdict/action language. |
| `/hypothesis` | `problem_classification.json`, `candidate_launchpad.json`, `portfolio_alternatives_builder.json`, same-run `candidate_generation.json` after explicit action. | Bounded Client Fit display summary as separate context; `run_metadata.json`; candidate factory provenance only for safety checks. | Full optimizer menu, Client Fit target mandates, unrelated candidate folders, stale candidate attempts, verdict artifacts. |
| `/comparison` | Same-run `current_vs_candidate.json`; same-run `candidate_generation.json`. | Bounded Client Fit display summary and target-reference context; product-scoped `candidate_comparison.json` behind adapter; output manifest paths. | Batch rankings, stale downstream comparison, selection verdict, suitability approval, fake metric rows. |
