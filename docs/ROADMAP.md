...
| RM-616 | In progress | Post-audit Sessions 12, 13 | Specify and implement the trade-off explanation and model-risk diagnostics layer. | RM-612 recommended. | [tradeoff and model risk spec](specs/tradeoff_and_model_risk_spec.md); comparison/selection/reporting modules | Spec done (Session 12): `tradeoff_explanation_v1` + `model_risk_diagnostics_v1`; implementation Session 13 | `python scripts/verify_docs.py`; Session 13: focused tests + report smoke. |
