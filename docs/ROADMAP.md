...
| RM-937 | Done | X-Ray Session 07 | Rework Portfolio Archetype as an evidence scorecard with conflicts and caveats. | RM-936. | [portfolio_xray.py](../src/portfolio_xray.py), [X-Ray spec](specs/portfolio_xray_diagnostics_spec.md), [PRODUCT](../PRODUCT.md) | Legacy `sections.portfolio_archetype` includes positive/negative evidence, confidence, conflicting signals (**not** Core MVP product surface; see spec §2.5) | `python -m pytest tests/test_portfolio_xray.py -q` (archetype V2 tests). |
