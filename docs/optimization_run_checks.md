...
## 5. “Топ‑5” актуальных изменений в политике оптимизатора

1. **RB по блокам — коридор, а не равенства.**  
   Внутри оптимизатора блоковые RC‑доли (Growth/Duration/Inflation) теперь реализуются как коридор `target ± 5 п.п.`, а не как строгие равенства. Пост‑hoc RB‑проверка во внешних скриптах использует это только диагностически (лог/снапшот), без дополнительного жёсткого стопа.

2. **RC‑слой упрощён до per‑asset RC cap + stress Top3.**  
   В оптимизации остались только глобальный per‑asset RC cap и блоковый RB‑коридор; HY/EM RC‑сублимиты убраны из жёстких ограничений. В stress‑тесте концентрация риска контролируется через Top1/Top3 RC в стресс‑ковариации.

3. **Явный fallback‑режим вместо полного FAIL.**  
   Если полный набор ограничений не может быть удовлетворён, оптимизатор строит детерминированный fallback‑портфель (капитал по блоковым RB, равные веса внутри блоков под каппингами) и возвращает его с флагами в статусе (`OK_FALLBACK`, `VIOL_RC_ASSET_CAP`, `FAIL_FEASIBILITY: ...`), а не пустой результат.

4. **Backtest: разделение production / research.**  
   В `config.yml` добавлен `backtest_mode` (`dynamic_nan_safe` по умолчанию / `simple` по желанию):  
   - `production` — простая NaN‑логика без перераспределения внутри блоков и RC‑gating (основной режим отчёта);  
   - `research` — полный NaN‑safe движок с within‑block перераспределением и RC‑gating (диагностический режим).

5. **Оптимизация: основное окно 10Y, 5Y — валидация; MaxDD — единственный жёсткий gate, Stress — диагностический.**  
   При наличии в `windows_months` 10‑летнего окна (120 мес.) оптимизация RB/RC и ProLiquidity выполняется на нём; если 10Y отсутствует, используется 5Y (60 мес.), а 3Y (`36`) для оптимизации не применяется. Параллельно, при наличии 5Y, тот же портфель проверяется на 5‑летнем окне (vol и RC по блокам логируются как валидационный слой, без дополнительного hard‑stop). Исторический MaxDD из шага оптимизации и отчёта остаётся единственным жёстким стоп‑критерием мандата, а результат Stress Judge носит диагностический характер и не блокирует запись весов и генерацию отчёта.
   Исторический MaxDD из шага оптимизации и отчёта остаётся единственным жёстким стоп‑критерием мандата. Результат Stress Judge (`PASS`/`FAIL_STRESS`) теперь не блокирует запись весов и генерацию отчёта, а используется как обязательный диагностический сигнал в логах, `stress_report.json` и snapshot‑файлах.

---

## 6. Краткий чеклист перед полным прогоном
