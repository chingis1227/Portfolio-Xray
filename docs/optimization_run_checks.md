...
1. **RB по блокам — коридор как quality gate (не стоп).**  
   Внутри оптимизатора блоковые RC‑доли реализуются как коридор `target ± 5 п.п.`. Пост‑оптимизационная проверка задаёт статус APPROVED vs CANDIDATE_RB_BREACH и флаг RB_BREACH с дельтами по блокам; веса **всегда записываются** (нет exit(1) при выходе из коридора).

2. **RC‑слой: предпочтительное соблюдение; при нарушении — флаг, веса возвращаются.**  
   Глобальный per‑asset RC cap по возможности соблюдается; при fallback без RC caps веса всё равно возвращаются, в run_result.json выставляются status OK_FALLBACK, violation VIOL_RC_ASSET_CAP и список тикеров с RC и cap. HY/EM RC‑сублимиты не в жёстких ограничениях. В stress‑тесте — Top1/Top3 RC в стресс‑ковариации.

3. **Явный fallback‑режим вместо полного FAIL.**  
   Если полный набор ограничений не может быть удовлетворён, оптимизатор строит детерминированный fallback‑портфель и возвращает его с флагами (`OK_FALLBACK`, `VIOL_RC_ASSET_CAP`); веса записываются, в run_result.json — нарушения и next_actions.
...
6. После оптимизации: проверить, что создались portfolio_weights.yml и run_result.json (status, violations, next_actions); затем запускать run_report.py.
