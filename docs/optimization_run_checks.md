...
|----------|------------|------------|
| Оптимизатор не сошёлся / жёсткий RC strict | См. лог и `run_result.json` (**FAIL_RC**, **FAIL_DATA** и т.д.) | Проверить `rc_asset_cap_pct`, min/max веса по бумагам, состав `tickers`, данные |
| Fallback после постобработки RC | Статус **OK_FALLBACK**, список `rc_breaches` | Оценить допустимость нарушений cap; при необходимости ослабить ограничения или расширить универсум |
...
2. **RC по активам:** глобальная формула cap (или явный `rc_asset_cap_pct`); постобработка и стресс смотрят **концентрацию по бумагам** (Top1 / Top3).
3. **Мандат MaxDD** на полной пересекающейся истории — единственный **жёсткий** барьер на запись весов наряду с **FAIL_DATA** / **FAIL_RC** (strict) и т.п.; стресс — **DIAG_***.
