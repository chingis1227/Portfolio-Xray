...
Историческое имя функции; по смыслу это **одностадийная** максимизация ожидаемой доходности с штрафами за отклонение от целевой vol/return и ограничениями по **весу** (см. `docs/specs/portfolio_construction_policy.md`, `docs/specs/feasibility_constraints_spec.md`). **RC_vol** не входит в ограничения оптимизатора.
...
- **run_optimization.py** — читает `config.yml`, `config/client_profiles.yml`, качает данные; пишет **`portfolio_weights.yml`**, **`run_result.json`**, стресс и снимки в `output_dir_final`. `stress_report.json` из optimization path теперь содержит не только factor betas/regressions, но и `factor_betas_stability`, `factor_covariance`, `factor_variance_decomposition`, historical factor attribution, и `portfolio_pca`.
...
4. **backtest_mode:** `dynamic_nan_safe` (по умолчанию) vs `simple` — см. `docs/specs/data_policy_spec.md`.
...
Если что-то падает — текст в консоли и таблицы выше; детали статусов — **`docs/specs/production_workflow.md`**.
