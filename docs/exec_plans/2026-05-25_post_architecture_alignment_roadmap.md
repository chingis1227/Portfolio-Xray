...
`removed root dirty-tree classification record` already classifies much of the dirty tree. Use it as an input, not as a source of truth that overrides current `git status`. It says migration-related code/docs/specs/tests should be kept for later review, generated artifacts should not be committed with migration work, `config.yml`, `config.yml.example`, and `requirements.txt` need human review, and unrelated IBKR/data-provider work should not be staged with Portfolio MRI architecture migration.
