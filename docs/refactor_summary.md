...
- **Optimization flow**: Load config → (optional) apply_profile_override → load data → apply_block_selection → run_risk_budget_optimization → RB corridor check (±5 pp) → ProLiquidity → MaxDD/Stress gatekeepers → write weights and run report only if portfolio valid.
...
| File | Changes |
|------|--------|
| **run_optimization.py** | Removed duplicate profile logic and `get_client_profile_from_config_file`. Single `apply_profile_override(cfg, args.profile)`. RB corridor check after optimization (exit 1 if outside ±5 pp). MaxDD and Stress Judge as gatekeepers: on FAIL do not write weights, do not run report, exit 1. Call to `apply_block_selection` before `run_risk_budget_optimization`. |
| **run_report.py** | `portfolio_returns_nan_safe` called with `blocks`, `rc_block_targets`, `rc_asset_cap_pct`, `cov_df`. `portfolio_valid` computed from Stress and MaxDD; passed to `export_run_metadata`; exit 1 if invalid. Added `sys` import. |
...
| **src/optimization.py** | `RB_CORRIDOR_PP = 0.05`, `rc_by_block_from_weights()`, `check_rb_corridor()`. Used in run_optimization after optimization. |
...
## 3. Hard constraints (gatekeepers)

- **Max DD**: If realized max drawdown is worse than `target_max_drawdown_pct`, portfolio is invalid. In optimization: no weight write, no report, exit 1. In report: `portfolio_valid = false` in metadata, exit 1.
- **Stress Judge**: If status is `FAIL_STRESS`, same as above.
- **RB corridor**: Realized block RC (Growth, Duration, Inflation) must be within target ± 5 pp. If not, optimization exits 1 and does not write weights.
